...
public class ConsecutivoCntService : BaseService, IConsecutivoCntService
{
    private string _actionUrl;
    private IConfiguration _cfg;
    public string _companyId;
    public ConsecutivoCntService(IHttpClientFactory httpClient,
        IConfiguration configuration
    ) : base(httpClient, configuration)
    {
        // configuration.GetValue<string>("ServicesUrl:Version")
        _actionUrl = string.Format("{0}consecutivoscnt"
    ...
    public Task<T> UpdateAsync<T>(string token, ConsecutivosCntDtoUpdate body)
    {
        return SendAsync<T>(new APIRequest()
        {
