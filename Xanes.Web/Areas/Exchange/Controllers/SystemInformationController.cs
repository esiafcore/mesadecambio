...
public class SystemInformationController : Controller
{
    ...
    public async Task<IActionResult> GetReport()
    {
        try
        {
            ...
            var reportDataVmTransFac = JsonSerializer.Deserialize<TransactionReportVM>(reportDataVmJson);

            switch (reportType)
            {
                case SD.ReportTransaType.Operation:
                    report = await SetReportDataForOperation(reportDataVmTransFac);
                    break;

                    //case SD.ReportTransaType.Deposit:
                    //    report = await SetReportDataForDeposit(reportDataVmTransFac);
                    //    break;

                    //case SD.ReportTransaType.Transfer:
                    //    report = await SetReportDataForTransfer(reportDataVmTransFac);
                    //    break;
            }
    ...
    private async Task<StiReport> SetReportDataForOperation(TransactionReportVM modelVm)
    {
        ...
        //reportResult.Dictionary.Variables[AC.ParFilterDescription].ValueObject = _parametersReport[ParametersReport.FiltersDescription]?.ToString() ?? string.Empty;
    ...
    public async Task<IActionResult> GetPartialView(SD.ReportTransaType reportType)
    {
        ...
        switch (reportType)
        {
            case SD.ReportTransaType.Operation:
                ...
                //        return BadRequest(resultResponse.ErrorMessages);
                //    }
                //    transaODTVM = resultResponse.Data as TransaODTVM;

                //    return PartialView(partialViewName, transaODTVM);
        ...
        try
        {
            ...
            foreach (var transaction in transactionList)
            {
                var transa = new TransaODTVM
                {
                    ...
                    ExchangeRateSellTransa = transaction.TypeNumeral == SD.QuotationType.Buy ? transaction.ExchangeRateBuyTransa : transaction.ExchangeRateSellTransa,
